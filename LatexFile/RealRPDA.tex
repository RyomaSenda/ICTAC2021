...
stack visibly manipulates its stack (or a {\em stack visibly} RPDA) if
...
\end{theorem}
{\bf Proof.}\quad
Assume $k$-RPDT $\calT$ over $\Sigma_\dblI,\Sigma_\dblO$ and $D$ satisfies $L(\calT)\subseteq S$.
Then we construct PDT $\calT'$ over $A_k^\dblI,A_k^\dblO$ and $\{z\}$ such that
$(q,a,\tst)\to(q',\asgn,j,\com)$ iff $(q,(a,\tst),z)\to(q',(\asgn,j,\com),\skip)$.
% where $\com'=\com$ if $v(\com)=\pop,\skip$ and $\com'=\push(z)$ if $v(\com)=\push$.
...
Next, assume an PDT $\calT'$ over $A_k^\dblI,A_k^\dblO$ and $\Gamma$
satisfies $L(\calT')\subseteq W_{S,k}$.
By the assumption, every $\overa\in L(\calT')$
satisfies $\Comp(\overa)\subseteq S$.
We construct a $k$-RPDT $\calT$ over $\Sigma_\dblI,\Sigma_\dblO$ and $D$
such that
$(q,a,\tst)\to(q',\asgn,j,\com)$ iff $(q,(a,\tst),z)\to(q',(\asgn,j,\com),\com')$ for some $z\in\Gamma$ and $\com'\in\Com(\Gamma)$.
For $w\in L(\calT)$, the compatible sequence
$\overa\in (A_k^\dblI\cdot A_k^\dblO)^\omega$ satisfies
$\overa\in L(\calT')$.
Because $\Comp(\overa)\subseteq S$ holds for $\overa\in L(\calT')$, $w\in S$ holds.
...
\begin{figure}[t]
  ...
  \caption{An example of transitions of $\calA$}
  \label{fig: lem_LkS}
\end{figure}
...
\end{theorem}
{\bf Proof.}\quad
By Lemma \ref{lem: LkS}, $L_{\overS,k}$ is definable by
a $(4,k+k'+4)$-DRPDA.
By Lemma \ref{the: RPDAtoPDA},
$\lab(L_{\overS,k})$ is definable by a $4$-DPDA.
Thus, $W_{S,k} = \overline{\lab(L_{\overS,k})}$ is also definable by
a $4$-DPDA $\calA_f$ by Lemma \ref{lem: W=lab}.
By Theorem \ref{the: finite_actions}, we can check \Real $($\DPDA, \PDT$)$ for $W_{S,k}$.
By Lemma \ref{lem: ef}, we can construct $0$-DPDA $\calA'_f$ such that
$L(\calA'_f)=L(\calA_f)$.
We can check \Real $($\DPDA, \PDT$)$ for $\calA'_f$,
