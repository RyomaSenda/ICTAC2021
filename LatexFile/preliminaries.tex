...
A run of TS $\calS=(S, s_0, A, E, \to_\calS, c)$ is
...
$\ef(w) = a_0a_1\cdots\in A^\infty$ be an epsilon free sequence of $w$
which is obtained by removing all symbols belong to $E$.
...
For $m\in\Natz$, we call $\calS$ $m$-TS
if all of runs $(\rho,w)$ of $\calS$ satisfy that
$w$ has no subsequence $w'\in E^{m+1}$.
